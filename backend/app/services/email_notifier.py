...
def _lead_rows(leads: list[dict], high_priority_score: int) -> str:
    ...
    for lead in leads:
        ...
        <tr>
          ...
          <td style="padding:14px;border-bottom:1px solid #e2e8f0">{_link(lead.get('website'), 'Visitar sitio')}</td>
          <td style="padding:14px;border-bottom:1px solid #e2e8f0">{_contact_html(lead)}</td>
          <td style="padding:14px;border-bottom:1px solid #e2e8f0;text-align:center"><span style="display:inline-block;padding:5px 8px;border-radius:6px;background:#fff1f2;color:{score_color};font-weight:800">{score}<br><span style="font-size:11px">({priority})</span></span></td>
        </tr>
        """)
    return "".join(rows)


def _detail_cards(leads: list[dict], high_priority_score: int, start_index: int = 1) -> str:
    cards = []
    for index, lead in enumerate(leads, start=start_index):
        analysis = lead["analysis"]
        score = analysis.score
        score_color = "#ef4444" if score >= high_priority_score else "#d97706" if score >= 50 else "#64748b"
        cards.append(f"""
...
def _build_daily_report_html(leads: list[dict], *, high_priority_score: int, searches: int, skipped_duplicates: int, errors: list[str]) -> str:
    now = datetime.now(ZoneInfo(settings.agent_timezone))
    scored = [lead for lead in leads if lead.get("analysis")]
    contactable = sorted([lead for lead in scored if lead.get("contactable")], key=lambda x: x["analysis"].score, reverse=True)
    unreachable = sorted([lead for lead in scored if not lead.get("contactable")], key=lambda x: x["analysis"].score, reverse=True)
    scores = [lead["analysis"].score for lead in scored]
    avg_score = round(sum(scores) / len(scores)) if scores else 0
    high_priority = sum(1 for lead in contactable if lead["analysis"].score >= high_priority_score)
    successful_searches = max(0, searches - len(errors))

    contactable_table = _lead_rows(contactable, high_priority_score) or '<tr><td colspan="5" style="padding:20px;text-align:center;color:#64748b">No se encontraron leads contactables nuevos.</td></tr>'
    unreachable_table = _lead_rows(unreachable, high_priority_score)
    unreachable_section = ""
    if unreachable:
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px"><tbody>{unreachable_table}</tbody></table>
        """
    ...
    <div style="max-width:760px;margin:24px auto;background:#ffffff;border:1px solid #dbe3ee;border-radius:8px;overflow:hidden">
      ...
      <div style="padding:30px">
        <div style="display:flex;border:1px solid #dbe3ee;border-radius:7px;background:#f8fafc;margin-bottom:28px">
          ...
          <div style="flex:1;padding:18px;text-align:center;border-left:1px solid #cbd5e1"><div style="font-size:12px;color:#64748b">ALTA PRIORIDAD CONTACTABLE</div><div style="font-size:24px;font-weight:800;color:#ef4444">{high_priority}</div></div>
        </div>

        <h2 style="font-size:17px;color:#1e293b;border-bottom:2px solid #e2e8f0;padding-bottom:10px">📋 Leads listos para contactar</h2>
        <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:14px"><thead><tr style="background:#f8fafc;color:#475569"><th style="padding:12px;text-align:left">Empresa</th><th style="padding:12px;text-align:left">Categoría</th><th style="padding:12px;text-align:left">Web</th><th style="padding:12px;text-align:left">Contacto</th><th style="padding:12px;text-align:center">Score</th></tr></thead><tbody>{contactable_table}</tbody></table></div>

        <h2 style="margin-top:30px;font-size:17px;color:#1e293b;border-bottom:2px solid #e2e8f0;padding-bottom:10px">💡 Análisis de oportunidades contactables</h2>
        ...
        {unreachable_section}
        {_errors_html(errors)}
        <p style="margin-top:26px;padding-top:16px;border-top:1px solid #e2e8f0;color:#94a3b8;font-size:12px">Búsquedas: {successful_searches}/{searches} completadas · Duplicados omitidos: {skipped_duplicates} · Sin contacto: {len(unreachable)} · Generado automáticamente por iWEB Marketing Agent.</p>
      </div>
    </div></body></html>
    """
...
def _build_daily_report_text(leads: list[dict], high_priority_score: int, errors: list[str]) -> str:
    scored = [lead for lead in leads if lead.get("analysis")]
    contactable = [lead for lead in scored if lead.get("contactable")]
    unreachable = [lead for lead in scored if not lead.get("contactable")]
    lines = ["iWEB Marketing Agent - Reporte diario", "", f"Leads contactables: {len(contactable)}", f"Sin contacto: {len(unreachable)}", ""]
    for lead in sorted(contactable, key=lambda item: item["analysis"].score, reverse=True):
        analysis = lead["analysis"]
        lines.extend([f"{lead.get('company')} - {analysis.score}/100", f"Rubro: {lead.get('industry')}", f"Web: {lead.get('website') or 'No disponible'}", f"Email: {lead.get('email') or 'No disponible'}", f"Teléfono: {lead.get('phone') or 'No disponible'}", f"Servicio sugerido: {analysis.recommended_service}", ""])
    if errors:
        lines.extend(["Búsquedas no completadas:", *[f"- {item}" for item in errors]])
    return "\n".join(lines)


def _send_daily_report_sync(leads: list[dict], high_priority_score: int, searches: int, skipped_duplicates: int, errors: list[str]) -> None:
    message = EmailMessage()
    message["From"] = settings.smtp_username
    message["To"] = settings.notification_email
    ...
    message.add_alternative(_build_daily_report_html(leads, high_priority_score=high_priority_score, searches=searches, skipped_duplicates=skipped_duplicates, errors=errors), subtype="html")
    _send_message_sync(message)


async def send_daily_lead_report(leads: list[dict], *, high_priority_score: int, searches: int, skipped_duplicates: int, errors: list[str] | None = None) -> None:
    await asyncio.to_thread(_send_daily_report_sync, leads, high_priority_score, searches, skipped_duplicates, errors or [])
